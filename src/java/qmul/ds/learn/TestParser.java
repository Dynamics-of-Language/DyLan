...
public class TestParser extends ContextParser {
	...
	/**
	 * @param corpusFile file of utterances to read in
	 * @param hasTargetFormulae whether the file has TTR record types or not
	 */
	...
	/**
	 * Method to write the incremental ttr output from parsing corpus, i.e. the word-by-word maximal semantics, to a file.
	 * Also outputs (additive) difference between output at previous word and that at the current one, including relevant dependent fields. e.g.:
	 * 
	 * uttnumber : (UTTID)
	 * [word]  		[currentTTR]    				         [diff]
	 * take         [x|take(x)]         			         [x|take(x)]
	 * the          [x|def(x)|take(x)]         				 [x|def(x)]
	 * red          [x|red(x)|def(x)|take(x)]  				 [x|red(x)]
	 * cross        [x|cross(x)|red(x)|def(x)|take(x)]       [x|cross(x)]
	 * 
	 * @param outputFile the file to write the corpus to
	 * @param errorFile the file to write all the parsing errors and summary at the end
	 * @param beam the number of parses to maintain in the parse state after each word, puts ceiling on decoding time
	 * @param hasUtteranceIndices boolean as to whether this file has an utterance index at the end of each line or not
	**/ 
		...
		corpusLoop : while (corpusIt.hasNext()) {
			...
			try { 
				...
				for (HasWord word : entry.first) {
					...
					//check to see if it hasn't managed a parse at all, despite backtracking. Shouldn't it output the previous one?
					if (!successful()){
					...
					stateMarker+=1;
					ParseState<ContextParserTuple> Best = getStateWithNBestTuples(1); //just outputs the current best TTR
	...
	public void test() {
		...
		corpusLoop : while (corpusIt.hasNext()) {
			...
			Pair<Sentence<Word>, TTRRecordType> entry = corpusIt.next();
			/*
			//skipping utterances with unknown words at test time or not?
	...
	public static void main(String a[]) {
		...
		tp.parseCorpusToFile("C:\\users\\julian\\TestTTRPentoCorpusErrors.txt","C:\\users\\julian\\TestTTRPentoCorpusRTs.txt",30,true);
	}
