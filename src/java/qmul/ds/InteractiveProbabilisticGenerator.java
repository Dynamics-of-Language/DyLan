...
public class InteractiveProbabilisticGenerator extends DAGGenerator<DAGTuple, GroundableEdge> {
	...
	static final String[] DSTypesString = GeneratorLearner.DSTypesString;//new String[]{"e", "t", "cn", "e>t", "cn>e", "e>cn", "e>(e>t)", "e>(t>t)", "e>(e>(e>t))"};
...
//			System.out.println(word +  row);
	...
	public List<WordLogProb> chooseTopWords(HashMap<String, Double> allProbs, Integer beamSize) {
...
//		TreeMap<Double, String> topWords = new TreeMap<>(Comparator.reverseOrder());
	...
	public List<WordLogProb> pickCandidates(List<TTRRecordType> mappedFeatures, TreeMap<String, TreeMap<Feature, Double>> model, HashMap<String, Double> wordProbs, Integer beamSize) {
		...
		for (String w: model.keySet()) {
			...
			for(String dsType: DSTypesString) {
				probSum += row.get(new Feature(DSType.parse(dsType), false));
				probSum += row.get(new Feature(DSType.parse(dsType), true));
			}
		...
		candidates = chooseTopWords(allProbs, beamSize);
		return candidates;
	}
		...
		try { // recommended by the IDE...
			...
			for (Pair<Sentence<Word>, TTRRecordType> pair : corpus){
				...
				while(!rInc.isEmpty()) {
					...
					mappedFeatures = mapFeatures(rInc, goalFeatures); // works ok. but why the warning for goalFeatures?
					List<WordLogProb> candidates = pickCandidates(mappedFeatures, learnedModel, wordProbs, BEAM); // works NOT ok!
