...
% builds new fo() value by beta-reduction
rule( elimination,
      tree( Nodes, Pointer ), tree( [node( Pointer, [ty(Type), fo(Formula) | Labels2] ) | Nodes2], Pointer ) ) :-
	select( node( Pointer, Labels ), Nodes, Nodes2 ),
...
% ELIMINATION
% given suitable ty(),fo() values and no outstanding requirements at 0- and 1-daughters,
% extends existing fo() value (mock-TTR-style) by beta-reduction
rule( elimination,
      tree( Nodes, Pointer ), tree( [node( Pointer, [ty(Type), fo(CFormula) | Labels2] ) | Nodes2], Pointer ) ) :-
	select( node( Pointer, Labels ), Nodes, Nodes2 ),
	select( ?ty(Type), Labels, LabelsTmp ),
	select( fo(OldFormula), LabelsTmp, Labels2 ),
	dtr( 0, Pointer, D0 ),
	dtr( 1, Pointer, D1 ),
	member( node( D0, Labels0 ), Nodes ),
	member( ty(Type0), Labels0 ),
	...
	\+ member( ?_Req1, Labels1 ),
	beta_reduce( Type1:Formula1, Type0:Formula0, Type:Formula ),
	conjoin( Type:OldFormula, Type:Formula, Type:CFormula ).
...
rule( cr-link-adjunction,
      tree( Nodes, Pointer ), tree( [node( LD, [?ty(e)] ) | Nodes], LD ) ) :-
	member( node( Pointer, Labels ), Nodes ),
	member( ty(e), Labels ),
	member( fo(_Formula), Labels ),
	linked_dtr( Pointer, LD ),
...
% LINK-EVALUATION
% at a node with a LINKed subtree below it, conjoin formulae
rule( link-evaluation,
	...
	select( ty(Type), Labels, Labels1 ), % book restricts to ty(t) or ty(cn)
	select( fo(Formula), Labels1, Labels2 ),
	dtr( 0, Pointer, D0 ), % book allows 1-daughter too for ty(t) only
	member( node( D0, _Labels0 ), Nodes ),
	linked_dtr( D0, DL ),
	member( node( DL, LabelsL ), Nodes ),
	member( ty(TypeL), LabelsL ), % book restricts to ty(t) or ty(cn)
