...
$genMessage = 'gen';
( $halt, $turn, $gen ) = ( 0, 0, 0 );
$args = '"setflag(format,html)." "setflag(io,sock)."';
$method = 'b_one';
$lex = 'no';
...
unless ( defined( $port ) ) {
    ...
    if ( $kid = fork ) {
	# parent process gives child time to start sicstus, then reads port value
	sleep( 1 );
	...
	# child process starts sicstus and dies heroically, redirecting output so CGI ends
	open STDOUT, '> /dev/null';
	system( "./ds $args &" );
	exit;
    }
    else {
	error_die( 'Can\'t fork' );
    }
...
<table width=100%>
<tr>
<td width=50% align=left><i>This implementation &copy; <a href="http://www.dcs.kcl.ac.uk/pg/purver/">Matthew Purver</a>, 2002-4</i></td>
<td width=50% align=right><i>Problems, comments, suggestions: <a href="mailto:matthew.purver\@kcl.ac.uk">matthew.purver\@kcl.ac.uk</a></i></td>
</tr>
</table>
END
